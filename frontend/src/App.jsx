...
function App() {
    ...
    try {
      ...
      try {
        ...
        if (genRes.ok) {
          ...
          // Fallback if generative quota or timeout occurs with explicit RAG policy compliance
    ...
    totalCampaigns > 0
      ? (
          campaignHistory.reduce(
            (total, campaign) => total + Number(campaign.score || 0),
            0
          ) / totalCampaigns
        ).toFixed(1)
      : "0.0";
  ...
  const readyToLaunchCount = campaignHistory.filter(
    (campaign) => campaign.status === "Ready to Launch"
  ).length;
  ...
  // =========================================================
  // MARK APPROVED CAMPAIGN AS READY TO LAUNCH
  // =========================================================
  ...
  const openCampaignFromNav = () => {
    ...
    if (!activeSegment) {
      setError("Analyze customer data and select a segment first.");
  ...
  // =========================================================
  // GENERATE CAMPAIGNS + EVALUATE (RESILIENT WITH FALLBACK)
  // =========================================================
  const generateCampaigns = async () => {
    ...
    try {
      // 1. Request 3 Variants from Backend
      ...
      <main className="main-content">
        ...
        {campaignResult && evaluationResult && (
          <section id="campaign-results" className="campaign-results-card">
            ...
            {/* CLEAN HUMAN REVIEW WITH STACKED LAUNCH ACTION */}
        ...
        {/* ===================================================
            CAMPAIGN HISTORY (WITH SCROLLABLE CONTAINER)
        =================================================== */}
                ...
                <article key={campaign.id} className="history-card">
                  <div className="history-main">
                    <div className="history-title-row">
                      <div>
                        <p className="history-product">{campaign.product}</p>
                    ...
                    <div className="history-meta">
                      <span>
                        Audience: <strong>{campaign.persona}</strong>
                      </span>
                      <span>
                        Platform: <strong>{campaign.platform}</strong>
                      </span>
                      <span>
                        Goal: <strong>{campaign.marketing_goal}</strong>
                      </span>
                      <span>
                        Variant: <strong>{campaign.variant_id}</strong>
                      </span>
                      {campaign.score !== null && (
                        <span>
                          AI Score: <strong>{campaign.score}/100</strong>
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="history-side">
                    ...
                    <small>{campaign.created_at}</small>
                  </div>
                </article>
              ))}
            </div>
                ...
                >
                  ...
                  {campaignHistory.map((campaign) => (
                    <option key={campaign.id} value={campaign.id}>
                      {campaign.product} — Variant {campaign.variant_id}
                    </option>
                  ))}
                </select>
              </div>
        ...
        {/* =========================================================
            EXPLAINABLE RETENTION ML WORKSPACE (STRICT 3-COLUMN GRID + RAG)
        ========================================================= */}
        <section
        ...
        >
          ...
          <div className="retention-grid-workspace">
            ...
            <div className="analytics-panel" style={{ background: "#111827" }}>
              ...
              {retentionCampaign ? (
                <div style={{ marginTop: "16px" }}>
                  {/* RAG VERIFIED CORPORATE POLICY BANNER */}
                  <div style={{
                    padding: "8px 12px",
